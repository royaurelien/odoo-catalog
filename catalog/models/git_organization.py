...
class GitOrganization(models.Model):
    ...
    auth_method = fields.Selection(
        [("public", "Public"), ("private", "Private")], default="public", required=True
    )
    ...
    repository_ids = fields.One2many(
        comodel_name="git.repository", inverse_name="organization_id"
    )
    repository_count = fields.Integer(
        compute="_compute_repository", compute_sudo=True, store=False
    )
    branch_count = fields.Integer(
        compute="_compute_repository", compute_sudo=True, store=False
    )
    custom_addons_count = fields.Integer(
        compute="_compute_repository", compute_sudo=True, store=False
    )

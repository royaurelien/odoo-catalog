...
class GitSync(models.AbstractModel):
    ...
    def _update_category(self, vals_list, categories):
        def apply(vals):
            ...
            if not category:
                res = dict(
                    filter(lambda item: search_key in item[0], categories.items())
                )
        ...
        if not hasattr(self, method):
            if raise_if_not_exists:
                raise NotImplementedError(
                    f"Method not implemented for {self._description} on {self.service}."
                )
    ...
    def _convert_to_odoo(self, item, **kwargs):
        return self.__get_method(
            "_convert_{}_to_odoo", self.service, {}, item, **kwargs
        )
    ...
    def _get_rules(self):
        ...
        if self._name != "git.organization":
            filtered_rules += rules.filtered(
                lambda rec: self.organization_id.id in rec.organization_ids.ids
            )

        actions = list(set(rules.mapped("action")))
        dict_rules = {
            action: rules.filtered(lambda rec: rec.action == action)
            ...
            try:
                ...
                if rule.action == "ignore":
                    ...
                    if condition:
                        ...
                        if rule.tag_ids:
                            vals.update(
                                {"tag_ids": [(4, tag.id) for tag in rule.tag_ids]}
                            )
                        if rule.partner_id:
    ...
    def _get_major_version_regex(self):
        regex = (
            ...
            .sudo()
            .get_param("catalog.odoo_major_version")
        )
    ...
    def _get_icon_search(self):
        res = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("catalog.enable_icon_search")
        )
    ...
    def _get_categories(self):
        ...
        for lang in languages:
            records = (
                self.env["ir.module.category"].with_context(**{"lang": lang}).search([])
            )
            categories.update(
                {self._format_category(record): record.id for record in records}
            )
    ...
    def _group_records_by_identidier(self, values):
        ...
        for records in records_by_sync:
            ...
            chunked_ids = [
                records.ids[i : i + values["job_count"]]
                for i in range(0, len(records.ids), values["job_count"])
            ]
    ...
    def action_update_commits(self):
        ...
        self.write(
            {
                "commit_ids": [(5, False, False)]
                + [(0, False, vals) for vals in vals_list]
            }
        )
        return True
    ...
    def _action_process(self, **kwargs):
        ...
        if self._name == "git.branch":
            ...
            vals_list = self._update_list_of_vals(
                vals_list, {"last_sync_date": datetime.now()}
            )
        ...
        vals_list = self._update_list_of_vals(
            vals_list, {"origin": f"{self.name} (Id: {self.id})"}
        )

        if self.partner_id and self.force_partner:
            vals_list = self._update_list_of_vals(
                vals_list, {"partner_id": self.partner_id.id}
            )
        ...
        if self._git_type_rel == "o2m":
            object_ids = {
                e[match_field]: e["id"] for e in self[rel_field].read([match_field])
            }
            to_update = [
                (1, object_ids.get(vals[match_field]), vals)
                for vals in vals_list
                if vals[match_field] in object_ids.keys()
            ]
        # More complex, search for all records from rel model
        elif self._git_type_rel == "m2m":
            object_ids = {
                e[match_field]: e["id"]
                for e in self.env[model_name].search([]).read([match_field])
            }
            to_update = [
                (4, object_ids.get(vals[match_field]))
                for vals in vals_list
                if vals[match_field] in object_ids.keys()
            ]
        ...
        to_create = [
            (0, False, vals)
            for vals in vals_list
            if vals[match_field] not in object_ids
        ]
        ...
        if force_update:
            to_update = {
                object_ids.get(vals[match_field]): vals
                for vals in vals_list
                if vals[match_field] in object_ids.keys()
            }
